# -*-coding:utf-8-*-
# 爬取B站舞蹈教学视频20210801-20211031时间段视频
import requests
import re
import json
import csv
...
if __name__ == '__main__':
        page=265
        file = open('dance1.csv', 'a', encoding='utf-8', newline='')
        # 先设置列名，并写入csv文件
        csv_writer = csv.DictWriter(file, fieldnames=["arcrank","arcurl","author","bvid","description","duration", "favorites","id","pic","play", "pubdate", "tag","title"])
        csv_writer.writeheader()
        for p in tqdm(range(page)):
            p=p+1
            # print(p)
            # break
            url="https://s.search.bilibili.com/cate/search?main_ver=v3&search_type=video&view_type=hot_rank&copy_right=-1&new_web_tag=1&order=click&cate_id=156&page={}&pagesize=30&time_from=20210801&time_to=20211031".format(p)
            # print(url)
            ...
            try:
                for n,i in enumerate(json_list):
                    ...
                    info={"arcrank":arcrank,"arcurl":arcurl,"author":author,
                          "bvid":bvid,"description":description,"duration":duration,
                    "favorites":favorites,"id":id,"pic":pic,"play":play,
                          "pubdate":pubdate, "tag":tag,"title":title}
