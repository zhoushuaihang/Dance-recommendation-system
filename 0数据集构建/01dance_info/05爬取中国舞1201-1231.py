...
if __name__ == '__main__':
    page =225
    file = open('ch_dance1201-1231.csv', 'a', encoding='utf-8', newline='')
    # 先设置列名，并写入csv文件
    csv_writer = csv.DictWriter(file, fieldnames=["id", "title", "tag", "description", "author", "bvid", "arcurl",
                                                  "favorites", "pic", "play", "pubdate"])
    csv_writer.writeheader()

    for p in range(page):
        p = p + 1
        url = "https://s.search.bilibili.com/cate/search?main_ver=v3&search_type=video&view_type=hot_rank&copy_right=-1&new_web_tag=1&order=scores&cate_id=200&page={}&pagesize=30&time_from=20211201&time_to=20211231".format(p)
        # print(url)
        ...
        try:
            for n, i in enumerate(json_list):
                ...
                info = {"id": id, "title": title, "tag": tag, "description": description, "author": author,
                        "bvid": bvid, "arcurl": arcurl, "favorites": favorites, "pic": pic, "play": play,
                        "pubdate": pubdate
                        }
                # print(info)
